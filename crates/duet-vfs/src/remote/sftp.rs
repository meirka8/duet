//! SFTP Remote Backend (Task T-7.1.3).
...
impl SftpFs {
    pub fn new(mount_id: MountId, profile_id: impl Into<String>) -> Self {
...
impl FileSystem for SftpFs {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities::READ | Capabilities::WRITE | Capabilities::STREAMING_LIST | Capabilities::REFLINK
    }
    ...
    async fn open_read(&self, p: &VPath) -> Result<Box<dyn AsyncReadSeek>> {
        ...
        if entries.contains_key(clean) {
            let content = b"SFTP remote file content\n";
    ...
    fn watch(&self, _p: &VPath) -> Result<BoxStream<'_, ChangeEvent>> {
        Err(VfsError::Unsupported("SftpFs watch unsupported".into()))
    }
