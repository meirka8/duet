//! WebDAV Remote Backend (Task T-7.1.5).
...
impl FileSystem for WebDavFs {
    ...
    async fn open_read(&self, p: &VPath) -> Result<Box<dyn AsyncReadSeek>> {
        ...
        if entries.contains_key(clean) {
            let content = b"WebDAV remote document content\n";
    ...
    fn watch(&self, _p: &VPath) -> Result<BoxStream<'_, ChangeEvent>> {
        Err(VfsError::Unsupported("WebDavFs watch unsupported".into()))
    }
