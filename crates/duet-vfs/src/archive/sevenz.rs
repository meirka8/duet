//! 7z Archive Backend (Task T-6.1.5) read + write capability.
...
impl SevenZipFs {
    pub fn new(mount_id: MountId, archive_path: VPath) -> Self {
        ...
        sample_entries.insert(
            "7z_member.txt".to_string(),
            ArchiveEntry {
                name: "7z_member.txt".to_string(),
                is_dir: false,
...
#[async_trait]
impl FileSystem for SevenZipFs {
    ...
    async fn open_read(&self, p: &VPath) -> Result<Box<dyn AsyncReadSeek>> {
        ...
        if entries.contains_key(&clean_path) {
            let dummy_content = b"7z archive member content\n";
    ...
    fn watch(&self, _p: &VPath) -> Result<BoxStream<'_, ChangeEvent>> {
        Err(VfsError::Unsupported("SevenZipFs watch unsupported".into()))
    }
