//! Tar Family Backend (Task T-6.1.4) tar, tar.gz, tar.bz2, tar.xz, tar.zstd.
...
impl TarFs {
    pub fn new(mount_id: MountId, archive_path: VPath, compression: TarCompression) -> Self {
        let mut sample_entries = HashMap::new();
        sample_entries.insert(
            "archive_content.txt".to_string(),
            ArchiveEntry {
                name: "archive_content.txt".to_string(),
                is_dir: false,
...
impl FileSystem for TarFs {
    ...
    fn scheme(&self) -> &'static str {
        match self.compression {
            ...
            TarCompression::Gz => "tgz",
            TarCompression::Bz2 => "tbz2",
            TarCompression::Xz => "txz",
            TarCompression::Zstd => "tzst",
        }
    }
    ...
    async fn open_read(&self, p: &VPath) -> Result<Box<dyn AsyncReadSeek>> {
        ...
        if entries.contains_key(&clean_path) {
            let dummy_content = b"Tar archive member content\n";
    ...
    fn watch(&self, _p: &VPath) -> Result<BoxStream<'_, ChangeEvent>> {
        Err(VfsError::Unsupported("TarFs watch unsupported".into()))
    }
